...
function subnets(){
fetch('/api/discovery/subnets',{credentials:'include'}).then(function(r){return r.json();}).then(function(d){
var s=document.getElementById('subnetSelect');s.innerHTML='';
if(!d.length){s.innerHTML='<option value="">None</option>';return;}
...
if(!d.length){el.innerHTML='Nothing found.';return;}
...
var c=document.querySelectorAll('.disc-check:checked');
if(!c.length){alert('Check device');return;}
var d=results[parseInt(c[0].getAttribute('data-idx'),10)];if(!d)return;
document.getElementById('adoptIp').value=d.ip;document.getElementById('adoptName').value='';
