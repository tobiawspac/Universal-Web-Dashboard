(function(){
var box=document.getElementById('devices'),q=document.getElementById('q'),devs=[],dm=false;
function load(){
if(dm)return Promise.resolve(window.__DEMO_DATA||[]);
return fetch('/api/dashboard-summary',{credentials:'include'}).then(function(r){return r.json();}).catch(function(){return[];});
}
function card(d){
...
function f(){
var v=q.value.toLowerCase().trim(),o=[];
if(!v)return devs;
for(var i=0;i<devs.length;i++){var x=devs[i];if((x.name||'').toLowerCase().indexOf(v)>=0||(x.ip||'').indexOf(v)>=0||(x.type||'').indexOf(v)>=0)o.push(x);}
return o;
}
function render(){
...
var on=0;
for(var i=0;i<devs.length;i++)if(devs[i].live&&devs[i].live.alive)on++;
...
var fl=f();
if(!fl.length){box.innerHTML='<p class="mono muted" style="padding:12px;text-align:center">No match</p>';return;}
...
document.getElementById('demoLink').onclick=function(e){e.preventDefault();dm=true;render();};
