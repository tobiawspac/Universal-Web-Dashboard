(function(){
function load(){fetch('/api/plugins',{credentials:'include'}).then(function(r){return r.json();}).then(function(d){
var el=document.getElementById('pluginsList');if(!d.length){el.textContent='No plugins.';return;}
...
var f=document.getElementById('pluginFile').files[0];if(!f){alert('ZIP?');return;}
...
if(r.ok){document.getElementById('uploadStatus').textContent='OK';load();}else document.getElementById('uploadStatus').textContent=r.error||'Fail';
