...
var code=document.getElementById('totpCode').value.trim();if(code.length!==6){alert('6 digits');return;}
...
document.getElementById('disable2faBtn').onclick=function(){if(!confirm('Disable?'))return;fetch('/api/settings/2fa/disable',{method:'POST',credentials:'include'}).then(function(){location.reload();}).catch(function(){});};
function sess(){fetch('/api/settings/sessions',{credentials:'include'}).then(function(r){return r.json();}).then(function(d){
var el=document.getElementById('sessionsList');if(!d.length){el.textContent='No sessions.';return;}
var h='';for(var i=0;i<d.length;i++)h+='<div style="margin-bottom:4px;padding:4px;font-size:11px">'+d[i].ip+' - '+d[i].userAgent+'</div>';el.innerHTML=h;
}).catch(function(){});}
