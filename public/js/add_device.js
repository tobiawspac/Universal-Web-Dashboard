...
function cfg(){var c={checkType:tc.value},p=document.getElementById('devicePort').value;if(p!=='')c.port=Number(p);var ph=document.getElementById('httpPath').value;if(ph!=='')c.httpPath=ph;c.https=document.getElementById('useHttps').checked;return c;}
...
document.getElementById('pingIp').onclick=function(){var b=this,ip=document.getElementById('deviceIP').value.trim();
if(!ip){msg('Enter IP',true);return;}b.disabled=true;b.textContent='Checking...';msg('Checking...');
var bd={host:ip,timeout:5000,checkType:cfg().checkType};if(cfg().port!=null)bd.port=cfg().port;if(cfg().httpPath!=null)bd.httpPath=cfg().httpPath;bd.https=cfg().https;
fetch('/check',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(bd),credentials:'include'})
.then(function(r){return r.json();}).then(function(r){if(r.alive)msg('Online'+(r.latencyMs!=null?' ('+r.latencyMs+' ms)':''));else msg('Offline',true);})
